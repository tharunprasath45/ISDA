...
function RecuiterDashboard() {
  ...
  return (
   <> <div>
     ...
     <p className='overview'>Overview of your job postings and applicants.</p>
    </div>
      ...
      >
        <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
          {stats.map((stat, index) => (
            <div key={index} style={{...threeboxes,
              border: clickbox === index? `2px solid ${stat.color}`:"1px solid #eef2f6"
            }} id="marketing-page">
              <h3 className="stat-label">{stat.title}</h3>
              <p className="stat-value" value={stats} onChange={(e)=> setStats(e.target.value)} >{stat.value}</p>
